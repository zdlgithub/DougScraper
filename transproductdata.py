...
try:
    ...
    product_file_path = os.path.join(settings.BASE_DIR,'datafiles','product_special_'+current_datetime+'.csv')
    ...
    for sku, meta_keywords, description, description_short, specification, title, meta_title, meta_description,\
        ...
        head_title = title + ' in ' + category_name3.replace('/', ' ').replace('AS ', '') + ' on athereshopping.com'
        ...
        product_string = product_id + '\;1\;' + title + '\;' + 'AS Special/' + category_name.replace('AS Lights & Lighting','AS LED Lights & Flashlights').replace('AS VR Special/','') + '\;' + \
